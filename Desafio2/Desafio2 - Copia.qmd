...
aggr(data_train, col = c('green','red'), numbers = TRUE, sortVars = TRUE, 
     labels = names(data_train), cex.axis = .5, gap = 2, 
     ylab = c("Proportion in variable","Proportion in dataset"))
```
...
Vimos anteriormente também que *Pmaquina1,* Pmaquina*2*, *Pmaquina3* tem um correlação linear alta de acima de 0.9, então usaremos arbitrariamente apenas *Pmaquina1.*
...
Analisando a variável relativa ao aluguel de máquinas pelos agricultores, vemos um certo desbalanceamento entre as diferentes máquinas, sendo a 2 a mais recorrente no conjunto de dados.
...
Abaixo apresentamos o número de máquinas alugadas na base de treino:
...
### Resposta vs Matéria Prima

De forma geral, o preço da máquina 1 e o da matéria prima não geram regiões de separação clara entre os tipos de máquinas alugadas. Assim, provavelmente um modelo apenas com essas duas variáveis não seja capaz de performar bem.
...
Ao comparar *CustoProduto* com *Pmaquinabase,* também não notamos divisões claras entre os tipos de máquina.

```{r}
hist_resp <- ggplot(data_train,
       aes(CustoProduto, Pmaquinabase, colour = as.factor(Resposta) )) +
  geom_point(stat="identity") +
  #geom_text(aes(label=paste0(sprintf("%1.1f", pct*100),"%")), position=position_stack(vjust=0.5)) +
  scale_colour_manual(guide_legend(title="Máquina Alugada"), values = c("#0072B2", "#D55E00", "#009E73")) + 
  #scale_x_discrete(guide = guide_axis(n.dodge=3))+
...
multinom_test <- postResample(data_test$Resposta, preds_class)[[1]]
```

Em seguida, aplicamos um procedimento de validação cruzada para estimar a acurácia geral do modelo no no conjunto completo de dados (treino + teste). Assim, ao treinamos o modelo em diferentes repartições de dados, temos uma estimativa mais assertiva do seu potencial preditivo.
...
multinom_cv <- mean(totalAccuracy)
```
...
Em seguida, implementar a abordagam bayesiana para o modelo multinomial.
...
data_trainb <- data_train 
levels(data_trainb$Resposta) <- paste0("L", levels(data_train$Resposta))
data_testb <- data_test
levels(data_testb$Resposta) <- paste0("L", levels(data_test$Resposta))

complete_datab <- complete_data
levels(complete_datab$Resposta) <- paste0("L", levels(complete_datab$Resposta))
...
p <- predict(b, newdata = data_testb, type = "parameter")
```
...
Sem o procedimento de validação cruzada, obtivemos 0.575 de acurácia.
...
for (cv in seq(1:cv)) {
  # assign chunk to data test
  dataTestIndex <- c((cv * cvDivider):(cv * cvDivider + cvDivider))
  dataTest <- complete_datab[dataTestIndex,]

  # everything else to train
  dataTrain <- complete_datab[-dataTestIndex,]

 b <- bamlss(f, family = "multinomial",
  data = dataTrain, reference = "L1" )
 
 p <- predict(b, newdata = dataTest, type = "parameter")
  ...
  probs1 <- probs %>% 
          mutate(max_prob = max.col(., ties.method = "last"),
          label =  as.integer(dataTest[,'Resposta']))
  
  probs1$max_prob<- as.factor(paste0("L", probs1$max_prob))
...
Nessa seção, aplicamos o algoritmo SMOTE para gerar dados simulados para as máquinas 1 e 3, já que são menos frequentes no conjunto de dados original.

```{r}

over_train <- SmoteClassif(Resposta ~ Prima + CustoProduto+ Pmaquinabase, data_train, k = 5, C.perc = list('1' = 1.75,'3' =1.75), repl = FALSE,
dist = "HEOM", p = 2)

over_test <- SmoteClassif(Resposta ~ Prima + CustoProduto+ Pmaquinabase, data_test, k = 5, C.perc = list('1' = 1.75,'3' =1.75), repl = FALSE,
dist = "HEOM", p = 2) 

over_complete <- over_train <- SmoteClassif(Resposta ~ Prima + CustoProduto+ Pmaquinabase, complete_data, k = 5, C.perc = list('1' = 1.75,'3' =1.75), repl = FALSE,
dist = "HEOM", p = 2)


hist_resp <- ggplot(over_train %>% count(Resposta) %>%    
         mutate(pct=n/sum(n)),
       aes(as.factor(Resposta), n, fill = Resposta)) +
  geom_bar(stat="identity") +
  scale_fill_manual(values = c("#0072B2", "#D55E00", "#009E73"))+
...
```{r}
teste <- rbind(over_train,data_test)
```

#### Modelo Multinomial

Abaixo repetimos a abordagem de conjunto de treino e teste para o modelo multinomial, porém agora temos um conjunto de treino balanceado.

```{r}
m1 <- nnet::multinom(Resposta ~ Prima + CustoProduto + Pmaquinabase + Regiao, data = over_train)

summary(m1)
```

Avaliando o desempenho do modelo no conjunto de teste original (sem balanceamento), conseguimos uma acurácia de 0.58.
...
postResample(data_test$Resposta, preds_class)
overmultinom_cv <- postResample(data_test$Resposta, preds_class)[[1]]
```

## Bibliografia

**Predicting Multiple Discrete Values with Multinomials, Neural Networkds and the {nnet} Package**. AMUNATEGI, Manueal. Disponível em: https://amunategui.github.io/multinomial-neuralnetworks-walkthrough/index.html. Acesso em: 15 de junho de 2023

**Multinomial Logistic Regression,** UCLA Statistical Methods and Data Analytics, Disponível em: https://stats.oarc.ucla.edu/r/dae/multinomial-logistic-regression/. Acesso em: 15 de junho de 2023

**Multiclass Classification with XGBoost in R,** RPubs, Disponível em: https://rpubs.com/mharris/multiclass_xgboost. Acesso em: 15 de junho de 2023.

**Beginners Tutorial on XGBoost and Parameter Tuning in R,** SARASWAT, Manish. Disponível em: https://www.hackerearth.com/practice/machine-learning/machine-learning-algorithms/beginners-tutorial-on-xgboost-parameter-tuning-r/tutorial/. Acesso em: 15 de junho de 2023.
