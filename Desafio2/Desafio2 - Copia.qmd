...
Procedimentos de amostragem e divisão de treino e teste estão disponível no arquivo .qmd.
...
Vimos anteriormente também que *Pmaquina2* e *Pmaquina3* tem um correlação linear alta de 0.94, então usaremos arbitrariamente apenas Pmaquina3, terminando com o seguinte conjunto de dados:

```{r}
df_reg <- data_train %>% 
  dplyr::select(c('Resposta', 'Pmaquina3', 'Prima'))
...
Inicialmente, escalamos os dados númericos para que diferentes dimensões não afetem a análise de agrupamento.
...
p1 = sns.scatterplot(data = df_cluster,y = 'CustoProduto', x = 'Pmaquina2',
hue = 'HCluster',
ax = ax[0])
p1.set(title = 'Prima por Pmaquina3',ylabel = 'Custo de Matéria Prima',xlabel = 'Preço da Máquina 3')
...
cluster = KMeans(random_state = 10727865, n_init= 10, n_clusters = 3)
...
p1 = sns.scatterplot(data = df_cluster,x = 'Cmaquina3', y = 'Pmaquina3',
hue = 'Resposta', ax = ax[0])
p1.set(title = 'Prima por Pmaquina3',xlabel = 'Custo de Matéria Prima',ylabel = 'Preço da Máquina 3')
plt.show()
...
p1 = sns.scatterplot(data = df_cluster,x = 'CustoProduto', y = 'Pmaquina3',
hue = 'KMeans',
ax = ax[0])
p1.set(title = 'Prima por Pmaquina3',xlabel = 'Custo de Matéria Prima',ylabel = 'Preço da Máquina 3')

# p2 = sns.scatterplot(data = df_cluster,x = 'Prima', y = 'Pmaquina3',
# hue = 'Resposta',
# ax = ax[1])
# p1.set(title = 'Prima por Pmaquina3',xlabel = 'Custo de Matéria Prima',ylabel = 'Preço da Máquina 3')
...
hist_resp <- ggplot(data_train,
       aes(Prima, Pmaquina3, colour = as.factor(Resposta) )) +
...
m1 <- multinom(Resposta2 ~ Pmaquina3 + Prima, data = df_reg)
