...
Em seguida, aplicamos um procedimento de validação cruzada para estimar a acurácia do modelo em todos os dados disponíveis (conjunto de treino + conjunto de teste).
...
Em seguida ajustamos um modelo RandomForest. Começamos ajustando os hiperparâmetros referente ao modelo e realizando um procedimento de validação cruzada no conjunto completo de dados (treino + teste). Usamos o mesmo conjunto de covariáveis selecionado no modelo multinomial (*Prima, CustoProduto, Pmaquinabase e Regiao*)
...
train_bayes <- data.frame(scale(data_train[, c('Pmaquinabase', 'Prima', 'Pmaquina1', 'Pmaquina2')]))
train_bayes <- cbind(train_bayes,make.names(data_train$Resposta))
colnames(train_bayes) <-  c('Pmaquinabase', 'Prima', 'Pmaquina1', 'Resposta')
train_bayes$Resposta <- as.integer(train_bayes$Resposta)

## Estimate.
b <- bamlss(f, family = "multinomial", data = train_bayes,reference = "X1")


train_bayes$Resposta[[1]]

scale(data.frame(data_train))
...
train_matrix <- xgb_data(data_train[, -1])
test_matrix <- xgb_data(data_test[,-1])
...
data_trainb <- data_train %>% 
   mutate(weights = case_when(Resposta == '1' ~ 1/0.27,
                           Resposta == '2' ~ 1/0.39,
                           Resposta == '3' ~1/0.17))
...
aux_train <- data_trainb[, c('Pmaquinabase', 'Prima', 'CustoProduto', 'Resposta')]
aux_test <- data_testb[, c('Pmaquinabase', 'Prima', 'CustoProduto', 'Resposta')]
trainbmatrix <- xgb_data(aux_train)
testebmatrix <- xgb_data(aux_test)



m1 <- xgboost(data = trainbmatrix, nrounds = 50, weight = data_trainb[, 'weights'],params = xgb_params)

#m1 <- xgb.train(data = trainbmatrix, nrounds = 50,params = xgb_params)


OOF_prediction <- data.frame(predict(m1, newdata = testebmatrix)) %>%
  mutate(Resposta = data_test[, 'Resposta'])
colnames(OOF_prediction) <- c('preds', 'Resposta')

confusionMatrix(factor(OOF_prediction$preds),
                factor(as.integer(OOF_prediction$Resposta)-1),
                mode = "everything")
```

```{r}
OOF_prediction <- data.frame(predict(m1, newdata = trainbmatrix)) %>%
  mutate(Resposta = data_train[, 'Resposta'])
colnames(OOF_prediction) <- c('preds', 'Resposta')
...
fact_col <- colnames(data_train)[sapply(data_train,is.character)]
...
data_train2 <- data_train[, c('Pmaquinabase', 'CustoProduto', 'Prima', 'Resposta')]
data_test2 <- data_test[, c('Pmaquinabase', 'CustoProduto', 'Prima', 'Resposta')]


traintask <- makeClassifTask (data = data_train2,target = "Resposta")
testtask <- makeClassifTask (data = data_test2,target = "Resposta")
...
```{r}
#create learner
lrn <- makeLearner("classif.xgboost",predict.type = "response", weights = T)
lrn$par.vals <- list( objective="multi:softprob", eval_metric="merror", nrounds=100L, eta=0.1)

?makeLearner

#getLearnerProperties(lrn)
#lrn$weights <- data_trainb[, 'weights']
...
```{r}
data_train_nn <- data_train %>% mutate_at(vars(Regiao), as.integer)
data_test_nn <- data_test %>% mutate_at(vars(Regiao), as.integer)

nn=neuralnet(Resposta ~Prima + CustoProduto + Pmaquinabase + Regiao,  
             data=data_train_nn, hidden=c(1,2,3,4,5,5,4,3,2,1), linear.output = FALSE)
```
...
```{r}
prediction <- data.frame(neuralnet::compute(nn,data.frame(data_test_nn[,-11]))$net.result) 
...
svm.tune <- train(x=data_train[, c('Pmaquinabase', 'Prima')],
                  y= make.names(data_train[,'Resposta']),
                  method = "mlpKerasDropoutCost",   # Radial kernel
