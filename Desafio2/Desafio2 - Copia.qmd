...
## Estimate.
b <- bamlss(f, family = "multinomial", data = data.frame(data_test),
  reference = "1")
...
nn=neuralnet(Resposta~Pmaquina1+Prima,  
             data=data_train, hidden=c(2,2), linear.output = FALSE)
```
...
```{r}
prediction <- data.frame(neuralnet::compute(nn,data.frame(data_train[,-11]))$net.result) 
