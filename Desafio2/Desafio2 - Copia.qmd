...
Ao verificar a acurácia, notamos que obtemos um bom desempenho, mas talvez ainda seja possível simplificar o modelo.

```{r}
test_pred <- predict(forest, complete_data)
postResample(complete_data$Resposta, test_pred)
rf <- postResample(complete_data$Resposta, test_pred)
```
...
Vemos que a Regiao é a característica menos relevante para a classificação. Assim, repetimos o procedimento de ajuste de hiperparâmetros e validação cruzada sem essa covariável.
...
nn=neuralnet(Resposta ~Prima + CustoProduto + Pmaquinabase + Regiao,  
             data=data_nn, hidden=c(1,2,3,4,5,5,4,3,2,1), linear.output = FALSE)
```
