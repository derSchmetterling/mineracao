...
Antes de analisarmos o comportamento dos dados no grupo de treino, vamos usar a concatenação das bases de treino e de teste após o tratamento de inconsistências para identificar perfis de compradores com base em um análise de agrupamento não hierárquica. Usamos a métrica conhecida como si
...
df <- rbind(data, data_test)
df <- data
#df_cluster <- read.csv2('baseprincipal.csv', sep = ',')
df_cluster <- df %>% mutate_at(vars(PRIVATE, URBAN, MEN,y), as.integer)
df_cluster$AGE <- scale(df_cluster$AGE)
df_cluster$SENIORITY <- scale(df_cluster$SENIORITY)


fviz_nbclust(df_cluster, hierarchical.clustering, method = "silhouette")


best_cluster <- NbClust(data = df_cluster, method = 'mcquitty')
dir(best_cluster)

# 
hierarchical.clustering <- hclust(dist(df_cluster), method = "mcquitty") 
# hc4 <- diana(df_cluster)

km <- kmeans(df_cluster, centers = 3)
df_cluster$HCluster <- km$cluster

#plot(hierarchical.clustering)

#names(hierarchical.clustering)
#hierarchical.clustering$call



# Cutting tree by no. of clusters
fit <- cutree(hierarchical.clustering, k = 2 )
table(fit)
# #rect.hclust(hierarchical.clustering, k = 3, border = "green")
df_cluster$HCluster <- fit
```

Analisando o agrupamento em relação a variável AGE, notamos que os clusters parecem estar divididos de acordo com as idades dos indivíduos. Condutores com menos de 40 anos recaem no cluster 2, condutores com 40 até aproximadamente 50 anos representam o cluster 1 e condutores com mais 60 anos foram adicionados ao cluster 3. Já em relação a SENIORITY, não temos claras divisões.

```{r}
plot_scatter <- function(df, x,y, yname){
  
  
  arg <- match.call()
  plot1 <- ggplot(df, aes(x=eval(arg$x), y = eval(arg$y), color=as.factor(HCluster))) +
    geom_point()+
    #scale_color_manual(values = c("#E69F00", "#56B4E9", "#009E73"),
    #                  labels = c('Cluster1', 'Cluster2', 'Cluster3'),
    #                  name = 'Clusters') +
    theme(legend.position="top") +
    labs(title = paste0('Distribuição de ', yname),
         x = yname,
...
df_cluster$SENIORITY <- df$SENIORITY
age <- plot_scatter(df_cluster, AGE, y, 'Idade')
sen <- plot_scatter(df_cluster, SENIORITY, y, 'Tempo de Carta')
...
Aplicando os agrupamentos para o conjunto de dados, não notamos nenhuma divisão relevante considerando as variáveis seguro, sexo e região de condução. De forma geral, temos uma menor proporção de indivíduos que adquiriram seguro completo, indivíduos do sexo feminino e indivíduos que dirigem pela zona urbana.

```{r}
#| warning: false
plot <- function(df, var, yname){

  
arg <- match.call()
plot1<- ggplot(df, aes(x=eval(arg$var), fill=as.factor(HCluster))) +
...
seg <- plot(df_cluster, y, 'Seguro')
sex <- plot(df_cluster, MEN, 'Sexo')
urban <-plot(df_cluster, URBAN, 'Região de Condução')
...
Já para PRIVATE, temos uma substancial diferença entre indivíduos que usam veículo privado

```{r}
plot(df_cluster, PRIVATE, 'Uso do Veículo')

df_private <-  df_cluster %>% filter(PRIVATE == 0)
...
A maior parte dos indivíduos dirigem em áreas rurais e não adquiriram o seguro completo. Também não notamos diferenças significativas entre a proporção de indivíduos com seguro completo que dirigem na área urbana.
...
Verificamos agora o ajuste do modelo utilizando os resíduos de quantil. De forma geral, se o modelo faz um bom ajuste dos dados, esperamos que apenas 5% dos pontos se encontrem fora do envelope simulado. Pelo envelope simulado gerado abaixo não notamos fortes desvios.
...
Abaixo, calculamos as métricas de acurácia, precisão, recall e f1score para os modelos loglog completo e reduzido.
...
# Reamostrando a base de dados

Como demonstrado na análise exploratória da base de treino, temos um desbalanceio entre o número de observações referentes a indivíduos que adquiriram ou não o seguro. Podemos reamostrar a base principal de 2000 observações usando o algoritmo SMOTE para gerar novas amostras para a classe minoritária (indivíduos que possuem seguro completo), de modo a balancear o conjunto de dados.

```{r}

########### Código para reamostrar classe minoritária#########
# set.seed(10727865)
# #| warning: false
# #install.packages('smotefamily')
# library(smotefamily)
# 
# data_full <- read.csv2('baseprincipal.csv', sep = ',')
# 
# 
# data_smote <- data_full %>%
#   mutate_at(vars("URBAN", "MEN", "PRIVATE"), as.factor)
#   
# 
# smote <- SMOTE(data_full[,-1], data_full$y, dup_size = 1)
# df_oversampled <- smote$data
# 
# write.csv(df_oversampled,'baseprincipalreamostrada.csv', sep = '/t', row.names = F)
######################################################3
```

Após a aplicação do algoritmo, temos um número mais equilibrado de indivíduos, a nova distribuição é apresentada abaixo.

```{r}


df_oversampled <- read.csv2('baseprincipalreamostrada.csv', sep = ',')


hist(as.integer(df_oversampled$class), col='coral', main = 'Distribuição de Seguro após SMOTE', xlab = 'Seguro',
     ylab = 'Frequência')
table(df_oversampled$class)


overdf_reg <- df_oversampled %>%
  mutate_at(vars("URBAN", "MEN", "PRIVATE", "SENIORITY", "AGE"), as.integer) %>%
  mutate_at(vars("URBAN", "MEN", "PRIVATE", "class"), as.factor) %>% 
  rename('y' = 'class')
...
Dividimos novamente o conjunto de dados entre treino e teste.

```{r}


########### Código para gerar teste e treino ###########
# set.seed(10727865)
# train_idx <- caret::createDataPartition(overdf_reg$y, p =0.7, list = FALSE)
# 
# train_df <- overdf_reg[train_idx,]
# test_df <- overdf_reg[-train_idx,]


# write.csv(train_df,'basetreinoreamostrada.csv', sep = '/t', row.names = F)
# 
# write.csv(test_df,'basetestereamostrada.csv', sep = '/t', row.names = F)
#######################################################

```

```{r}
train_df  <- read.csv2('basetreinoreamostrada.csv', sep = ',')
test_df  <- read.csv2('basetestereamostrada.csv', sep = ',')
...
```{r}
m0_over <- glm(formula = y ~  URBAN + SENIORITY+ AGE + MEN,
...
```{r}
hnp.fit.modell = hnp(m0_over, print.on=TRUE, plot=FALSE,
halfnormal=F)
## Binomial model
...
```{r}
y_pred <- predict(m0_over,newdata = test_df, type = 'response')
roc_logit <- Epi::ROC(y_pred, test_df$y, plot= T)

logitra_res_test <- accuracy(m0_over, roc_logit, test_df )
logitra_acc_test <- logitra_res_test[1]
logitra_f1_test<-  logitra_res_test[4]
logitra_precision_test <- logitra_res_test[2]
logitra_recall_test <- logitra_res_test[3]
```
