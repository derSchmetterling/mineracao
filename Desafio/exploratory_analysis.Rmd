...
Antes de analisarmos o comportamento dos dados no grupo de treino, vamos usar o arquivo baseprincipal.csv para identificar perfis de compradores com base em um análise de agrupamento hierárquica.
...
df = pd.read_csv('baseprincipal.csv')
...
Para definir o número clusters adequados, usamos uma métrica conhecida como *silhoutte score*: uma medida que varia de -1 a 1 e que mensura quão próximo as observações de um grupo estão de outros grupos. Idealmente queremos valores o mais próximo possível de 1.
...
visualizer = KElbowVisualizer(model, k=(3,12), timings=False, metric = 'silhouette')

visualizer.fit(df)        # Fit the data to the visualizer
visualizer.show()        # Finalize and render the figure
```

Usando 3 agrupamentos, temos o seguinte resultado:
...
print('Número de clusters: ', len(df_cluster['HCluster'].unique()))

for i in df_cluster['HCluster'].unique():
    siz = len(df_cluster[df_cluster['HCluster'] == i])
    print(f'Cluster {i} tem {siz} indivíduos')

```

Nos resta agora interpretar os agrupamentos com bases nas variáveis:
...
def plots(data, x_axis, y_axis, hue,yaxis_title, xaxis_name, yaxis_name, colorblind = False):
        if colorblind:
        ...
        p2 = sns.histplot(data = data,
                        x = y_axis,
                        hue = hue, 
                        discrete = True,
                        multiple = 'stack')
        p2.set(title = f'Distribuição {yaxis_title}',
                xlabel = yaxis_name,
                ylabel = 'Frequência')
        
        aux01 = len(data[(data['HCluster'] == '0') & (data[y_axis] == 0)])
        aux01_total = len(data[(data['HCluster'] == '0')])


        aux02 = len(data[(data['HCluster'] == '1') & (data[y_axis] == 0)])
        aux02_total = len(data[(data['HCluster'] == '1')])


        aux03 = len(data[(data['HCluster'] == '2') & (data[y_axis] == 0)])
        aux03_total = len(data[(data['HCluster'] == '2')])


        c1 = f'Cluster 1: {aux01}/{aux01_total}'
        c2 = f'Cluster 2: {aux02}/{aux02_total}'
        c3 = f'Cluster 0: {aux03}/{aux03_total}'
        plt.legend(title = f'Observações com {yaxis_title} igual a 0',labels=[c1, c2,c3], bbox_to_anchor=(-0.1,1.))
        rcParams['figure.figsize'] = 5.7,10.27
...
plots(df_cluster, 'AGE', 'y', 'HCluster', 'Seguro', 'Idade', 'Cobertura Completa(0 - Não, 1 - Sim)')
```
...
## Binomial model
glmtoolbox::envelope(m1_cauchit)
