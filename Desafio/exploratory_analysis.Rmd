...
train_dfb <- train_df %>% 
  ...
  mutate_at(vars('y'), as.integer)

N = nrow(train_dfb)
jagsData <- list(N = N, y = train_dfb$y,
    URBAN = train_dfb$URBAN, SEN = train_dfb$SENIORITY, 
    AGE = train_dfb$AGE, MEN = train_dfb$MEN)
```

```{r}
m1_blogita <- jags(data=jagsData,model.file='models/m1_cauchit.bug',
                   param=c('beta0','beta_urban', 'beta_sen', 'beta_age', 'beta_men', 'lambda'),
                   n.chains=3, n.iter=10000, n.burnin=5000, n.thin=10)
```
...
mcmc.samples <- as.mcmc(m1_blogita)
densityplot(mcmc.samples)
...
```{r}
pred_test <- predict_bcauchit(mcmc.samples,test_df)

roc_bayes <- Epi::ROC(pred_test, test_df$y, plot= F)


metrics_bayes <- accuracy_bayes(roc_bayes, test_df, pred_test )
acc_bayes <- metrics_bayes[1]
prec_bayes <- metrics_bayes[2]
recall_bayes<-metrics_bayes[3]
f1_bayes <- metrics_bayes[4]
