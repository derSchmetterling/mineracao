...
As colunas que podem apresentar algum tipo de inconsistência são Age e Seniority. Um indivíduo não poderia ter mais anos de trabalho em uma mesma empresa do que de vida.
...
Felizmente não temos esse problema em nossa amostra. Filtramos também indivíduos menores de idade e encontramos duas observações. Não sabemos de qual país os dados foram retirados, contudo a
...
O intercepto e o coeficiente de Private são não significativos, logo retiramos seus coeficientes do modelo.
...
```{r}
m1_probit <- glm(formula = y ~  URBAN + SENIORITY+ AGE + MEN-1,
...
Usando a função de ligação Cauchit, temos uma leve melhor na área sob a curva obtendo 0.786, porém continuamos obtendo indicativos de mau ajuste.
...
m1_loglog <- glm(formula = y ~  URBAN + SENIORITY+ AGE + MEN-1,
...
A seguir,testamos o poder de predição de todos os modelos ajustados até o momento.
...
roc_logit <- ROC(y_pred, data_test$y, plot= F)
...
logitr_acc_train <- accuracy(ajuste3, roc_logit, data_test )
```

```{r}
## probit model
y_pred <- predict(m1_probit,newdata = data_test, type = 'response')
roc_probit <- ROC(y_pred, data_test$y, plot= F)
...
roc_loglog <- ROC(y_pred, data_test$y, plot= F)
...
roc_cauchit <- ROC(y_pred, data_test$y, plot= F)
...
roc_cloglog <- ROC(y_pred, data_test$y, plot= F)
...
data <- data.frame(
  Modelo = c("Modelo logit", "Modelo probit", "Modelo cauchit", "Modelo cloglog", "Modelo loglog", "Modelo loglog Reduzido"),
  AIC =  c(AIC(m1_logit),AIC(m1_probit),AIC(m1_cauchit),
AIC(m1_cloglog), AIC(m1_loglog), AIC(ajuste3)),
  Acuracia_Teste = c(logit_acc_train,probit_acc_train,cauchit_acc_train,
cloglog_acc_train, loglog_acc_train, logitr_acc_train)
)
...
Comparando tanto o AIC quanto a acurácia, vemos que o modelo com função de ligação loglog completo e o reduzido possuem um desempenho melhor do que os outros.
