...
full_data <-  rbind(data, data_test)
```
...
numeric_full <- full_data %>% 
  mutate_at(vars(numeric), as.integer)
...
hist_men <- ggplot(full_data %>% count(y, MEN) %>%    
         mutate(pct=n/sum(n)),
...
```{r}
print(paste('Indíviduos sem cobertura completa',nrow(filter(full_data, y == '0'))))
print(paste('Indíviduos com cobertura completa',nrow(filter(full_data, y == '1'))))
```
...
```{r}
hist_urban<- ggplot(full_data %>% count(y, URBAN) %>%    
         mutate(pct=n/sum(n)),
...
```{r}
hist_private<- ggplot(full_data %>% count(y, PRIVATE) %>%    
         mutate(pct=n/sum(n)),
...
```{r}
p1 <- ggplot(full_data, aes(x=as.factor(y), y=AGE)) + 
...
p2 <- ggplot(full_data, aes(x=as.factor(URBAN), y=AGE)) + 
...
p3 <- ggplot(full_data, aes(x=as.factor(PRIVATE), y=AGE)) + 
...
p4 <- ggplot(full_data, aes(x=as.factor(MEN), y=AGE, color = as.factor(y))) + 
...
```{r}
p1 <- ggplot(full_data, aes(x=as.factor(y), y=SENIORITY)) + 
...
p2 <- ggplot(full_data, aes(x=as.factor(URBAN), y=SENIORITY)) + 
...
p3 <- ggplot(full_data, aes(x=as.factor(PRIVATE), y=SENIORITY)) + 
...
p4 <- ggplot(full_data, aes(x=as.factor(MEN), y=SENIORITY)) + 
...
```{r}
library(patchwork)
scatter_age <- ggplot(full_data, aes(y=y,
                x=log(AGE), color = as.factor(MEN)), pch = 21) +
...
scatter_urban <- ggplot(full_data, aes(y=y,
                x=AGE**2, color = as.factor(URBAN)), pch = 21) +
geom_point(position="identity", alpha=0.5) +
ylab("Seguro") +
xlab("Idade") +
ggtitle("Distribuição das idades") + 
...
# scatter_private<- ggplot(full_data, aes(y=y,
...
patch_g3g4 <- scatter_age/scatter_urban
patch_g3g4
```

# Modelos Lineares Generalizados (Clássicos)

Nessa seção, temos o objetivo de ajustar o melhor modelo para predizer, dado as informações do indivíduo, se ele possui ou não cobertura completa do seguro. Usamos a divisão de treino e teste para validar o melhor modelo e aplicamos valição cruzada para testar seu desempenho.
...
Epi::ROC(m1_loglog$fitted.values, data$y, plot= "F")
```
...
Essa seção implementa a validação cruzada para testar o desempenho do modelo loglog em diferentes partições de dados.

```{r, message=FALSE, warning=FALSE, echo=FALSE, include=FALSE}
totalAccuracy <- data.frame(Acc = c(), Precision = c(), Recall = c(), F1 = c())
cv <- 10
#cv <- 224
cvDivider <- floor(nrow(full_data) / (cv+1))
#cvDivider <- 50

factors <- c("y", "MEN", "URBAN", "PRIVATE")

full_data_loglog <- full_data %>% 
...
for (cv in seq(1:cv)) {
  ...
  dataTest <- full_data_loglog[dataTestIndex,]

  # everything else to train
  dataTrain <- full_data_loglog[-dataTestIndex,]

 m1_loglog_cv <- glm(formula = y ~  URBAN + SENIORITY+ AGE + MEN,
 family = binomial(link = loglog()), data = dataTrain)
 
  y_pred <- predict(m1_loglog_cv,newdata = dataTest, type = 'response')
 

  roc_loglog <- Epi::ROC(y_pred, dataTest$y, plot= F)

  metrics <- accuracy(m1_loglog_cv, roc_loglog, dataTest )
  to_bind <- data.frame(Acc = c(metrics[1]), Precision = c(metrics[2]), Recall = c(metrics[3]), F1 = c(metrics[4]))

  #  classification error
  #cv_ac <- postResample(dataTest$Resposta, probs1$max_prob)[[1]]
  print(paste('Current Recall:',metrics[3],'for CV:',cv))
  totalAccuracy <- rbind(totalAccuracy, to_bind)
}
```

```{r}
mean_metrics <- summarize_all(totalAccuracy, mean)

loglog_acc_cv <- mean_metrics[1]
loglog_f1_cv<-  mean_metrics[4]
loglog_precision_cv <- mean_metrics[2]
loglog_recall_cv <- mean_metrics[3]
```
...
metrics_rf <- function(model, data_test) {
...
metrics <- c(acc_rf, precision_forest, recall_forest,f1_forest )
...
fulldata_rf <- full_data %>% 
  mutate_at(vars('y', 'MEN', 'PRIVATE','URBAN'), as.factor)
```
...
cv <- 10
#cv <- 224
cvDivider <- floor(nrow(fulldata_rf) / (cv+1))
#cvDivider <- 50

for (cv in seq(1:cv)) {
  # assign chunk to data test
  dataTestIndex <- c((cv * cvDivider):(cv * cvDivider + cvDivider))
  dataTest <- fulldata_rf[dataTestIndex,]

  # everything else to train
  dataTrain <- fulldata_rf[-dataTestIndex,]

  rf <- randomForest(y~., data=dataTrain, proximity=TRUE, mtry = 2)
 
  metrics <- metrics_rf(rf, dataTest)
...
```{r}
mean_metrics_rf <- summarize_all(totalAccuracy, mean)

rf_acc_cv <- mean_metrics_rf[1]
rf_f1_cv<-  mean_metrics_rf[4]
rf_precision_cv <- mean_metrics_rf[2]
rf_recall_cv <- mean_metrics_rf[3]
```
...
```{r}
# data_bayes <- data_test %>%
#  mutate_at(vars('y', 'MEN', 'URBAN'), as.character) %>%
#  mutate_at(vars('y', 'MEN', 'URBAN'), as.integer)
#data_scaled = data
#data = over_train
data_scaled = data
data_scaled$AGE <- as.numeric(scale(data_scaled$AGE))
data_scaled$SENIORITY <- as.numeric(scale(data_scaled$SENIORITY))
...
m1_blogit <- jags(data=jagsData,model.file='models/m1_cauchit.bug',
                   ...
                   n.chains=2, n.iter=15000, n.burnin=5000, n.thin=10)
```
...
predict_bcauchit <- function(samples, data) {
# data deve ser do tipo dataframe

#vetor da média dos coeficientes beta_age/beta_men/beta_sen/beta_urban
coef <- as.matrix(summary(samples)$statistics[, 'Mean'][1:5])
lambda <- as.matrix(summary(samples)$statistics[, 'Mean'])[7]
...
pred_test <- predict_bcauchit(samples,data_test)
...
xgb_data <- function(data){
  ...
  data<- data %>%
    dplyr::select(c('MEN', 'URBAN', 'AGE', 'SENIORITY', 'y')) %>% 
    mutate_at(vars(y), as.character) %>% 
    mutate_at(vars(MEN, URBAN,y), as.integer) 
   
  data_variables <- as.matrix(data[!(names(data) %in% 'y')])
  data_label <- as.integer(data[,'y'])
...
train_matrix <- xgb_data(data_reg)
test_matrix <- xgb_data(data_test)

full_matrix <- xgb_data(full_data)

```

```{r}

negative_cases <- sum(data_reg$y == '0')
postive_cases <- sum(data_reg$y == '1')

model <- xgboost(data = train_matrix,                    # the data   
                 max.depth=50,                           # max depth 
                 nrounds=50,
...
roc_xgb <- Epi::ROC(pred, data_test$y, plot = 'F')
thresh_index <- which.max(rowSums(roc_xgb$res[, c("sens", "spec")]))
thresh <- as.double(rownames(roc_xgb$res[thresh_index,][1]))
...
```{r}
xgb_metrics <- function(model, pred, data_test) {
  
  ## xgb model, pred its the prediction from test_text matrix and data_test is the the data in data.frame format
  ...
  cf_test <- confusionMatrix(factor(pred_class),
                factor(data_test$y),
                mode = "everything")
  
  acc_xgb <- cf_test$overall[1]


  #f1 score
  #recall or sensitity/ recall or PPV
  recall_xgb <- as.double(cf_test$byClass[1])
  precision_xgb <- as.double(cf_test$byClass[3])
  
  f1_xgb <- 2*(recall_xgb*precision_xgb)/(precision_xgb+recall_xgb)
  
  metrics <- c(acc_xgb, precision_xgb, recall_xgb,f1_xgb )
...
for (cv in seq(1:cv)) {
  ...
  train_matrix <- xgb_data(dataTrain)
  test_matrix <- xgb_data(dataTest) 
  ...
  metrics <- xgb_metrics(model, pred, dataTest)
...
```{r}
mean_metrics_xgb <- summarize_all(totalAccuracy, mean)

xgb_acc_cv <- mean_metrics_xgb[1]
xgb_f1_cv<-  mean_metrics_xgb[4]
xgb_precision_cv <- mean_metrics_xgb[2]
xgb_recall_cv <- mean_metrics_xgb[3]
```
...
model{
  for (i in 1:N) {
    y[i] ~ dbern(p[i])
    m[i] <- beta0+beta_urban*URBAN[i] + beta_sen*SEN[i] + beta_age*AGE[i] + beta_men*MEN[i] + beta_private*PRIVATE[i]
    pstar[i] <- 1/3.141592* arctan(-m[i]) + 1/2
    p[i] <- 1-pow(pstar[i], lambda)
  ...
  beta_age ~ dnorm(0,100)
  beta_men ~ dnorm(0,100)
  beta_private ~ dnorm(0,100)
  
  }

"
writeLines(modelString, con='models/m1_cauchit2.bug')
```
...
# N = nrow(data_scaled)
# jagsData <- list(N = N, y = data_scaled$y,
#     URBAN = data_scaled$URBAN, SEN = data_scaled$SENIORITY,AGE = data_scaled$AGE, MEN = data_scaled$MEN, PRIVATE = data_scaled$PRIVATE)
```

```{r, echo=FALSE, message=FALSE}
#| echo: false
# m1_blogit <- jags(data=jagsData,model.file='models/m1_cauchit2.bug',
#                    param=c('beta0','beta_urban', 'beta_sen', 'beta_age', 'beta_men', 'beta_private', 'lambda'),
#                    n.chains=2, n.iter=15000, n.burnin=5000, n.thin=10)
```
...
# # data deve ser do tipo dataframe
# 
# #vetor da média dos coeficientes beta_age/beta_men/beta_sen/beta_urban/beta_private
# coef <- as.matrix(summary(samples)$statistics[, 'Mean'][1:6])
# lambda <- as.matrix(summary(samples)$statistics[, 'Mean'])[8]
# 
# 
# data_matrix <- data.matrix(data_test)[,c(-1)][, c(4,1,3,5,2)]
# data_matrix <- cbind(data_matrix, intercept = 1)
# n <- dim(data_matrix)[1]
# 
# 
...
#   
#   
#   thresh <- roc_bayes$res[thresh_index,][1,5]
#   
#   predict <- ifelse(predict > thresh, '1', '0')
#   
#   acc <- mean(predict == as.character(test_df$y))
#   
#   return(c(acc,precision, recall,f1_score))
#   
# }
...
# 
# 
# roc_bayes <- Epi::ROC(pred_test, data_test$y, plot= 'sp')
# 
# 
# thresh_index <- which.max(rowSums(roc_bayes$res[, c("sens",'spec')]))
# 
...
# 
# metrics_bayes <- accuracy_bayes(roc_bayes, data_test, pred_test )
# acc_bayes_2 <- metrics_bayes[1]
# prec_bayes_2 <- metrics_bayes[2]
# recall_bayes_2<-metrics_bayes[3]
# f1_bayes_2 <- metrics_bayes[4]

```

# Métodos de Reamostragem

Para lidar com o desbalanceamento entre as classes da variável resposta, aplicaremos o método SMOTE na base de dados de treino.

```{r}
library(UBL)

over_train <- SmoteClassif(y ~ MEN + URBAN+ AGE + SENIORITY, fulldata_rf, k = 5, C.perc = list('1' = 2,'0' =1), repl = FALSE,dist = "HEOM", p = 2)

over_train <- over_train[sample(nrow(over_train)),]

```

```{r}
hist_men <- ggplot(over_train %>% count(y, MEN) %>%    
         mutate(pct=n/sum(n)),
       aes(as.factor(y), n, fill=as.factor(MEN))) +
  geom_bar(stat="identity") +
  geom_text(aes(label=paste0(sprintf("%1.1f", pct*100),"%")), position=position_stack(vjust=0.5)) +
...
```{r}
over_loglog <- glm(formula = y ~  URBAN + SENIORITY+ AGE + MEN,
 family = binomial(link = loglog()), data = over_train)
summary(over_loglog)
```
...
cv <- 10
#cv <- 224
cvDivider <- floor(nrow(over_train) / (cv+1))
#cvDivider <- 50

for (cv in seq(1:cv)) {
  # assign chunk to data test
  dataTestIndex <- c((cv * cvDivider):(cv * cvDivider + cvDivider))
  dataTest <- over_train[dataTestIndex,]

  # everything else to train
  dataTrain <- over_train[-dataTestIndex,]

 over_loglog <- glm(formula = y ~  URBAN + SENIORITY+ AGE + MEN,
 family = binomial(link = loglog()), data = dataTrain)
 
  y_pred <- predict(over_loglog,newdata = dataTest, type = 'response')
 

  roc_loglog <- Epi::ROC(y_pred, dataTest$y, plot= F)

  metrics <- accuracy(over_loglog, roc_loglog, dataTest )
  to_bind <- data.frame(Acc = c(metrics[1]), Precision = c(metrics[2]), Recall = c(metrics[3]), F1 = c(metrics[4]))
...
```{r}
mean_loglog_smote <- summarize_all(totalAccuracy, mean)

loglog_acc_smote <- mean_loglog_smote[1]
loglog_f1_smote<-  mean_loglog_smote[4]
loglog_precision_smote <- mean_loglog_smote[2]
loglog_recall_smote <- mean_loglog_smote[3]
```

```{r}
# y_pred <- predict(over_loglog,newdata = data_test, type = 'response')
# roc_logit <- Epi::ROC(y_pred, data_test$y, plot= T)
# 
# logitr_res_test <- accuracy(over_loglog, roc_logit, data_test )
# logitr_acc_test <- logitr_res_test[1]
# logitr_f1_test<-  logitr_res_test[4]
# logitr_precision_test <- logitr_res_test[2]
# logitr_recall_test <- logitr_res_test[3]
...
```{r}
rf <- randomForest(y~., data=over_train, proximity=TRUE, mtry = 2) 
print(rf)
```
...
```{r}
mean_rf_smote <- summarize_all(totalAccuracy, mean)

rf_acc_smote <- mean_rf_smote[1]
rf_f1_smote<-  mean_rf_smote[4]
rf_precision_smote <- mean_rf_smote[2]
rf_recall_smote <- mean_rf_smote[3]
```
...
for (cv in seq(1:cv)) {
  ...
  negative_cases <- sum(dataTrain$y == '0')
  postive_cases <- sum(dataTrain$y == '1')
...
```{r}
metrics_xgb_smote <- summarize_all(totalAccuracy, mean)

xgb_acc_smote <- metrics_xgb_smote[1]
xgb_f1_smote<-  metrics_xgb_smote[4]
xgb_precision_smote <- metrics_xgb_smote[2]
xgb_recall_smote <- metrics_xgb_smote[3]
```
...
data <- data.frame(
  Modelo = c("Modelo loglog", "Loglog SMOTE" ,"Random Forest", "Random Forest SMOTE","XGBoost", "XGBoost SMOTE", "Cauchit Potência Inversa"),
  'Acurácia' = round(c(loglog_acc_cv[1,1], loglog_acc_smote[1,1], rf_acc_cv[1,1],rf_acc_smote[1,1], xgb_acc_cv[1,1], xgb_acc_smote[1,1],acc_bayes),3),
  'Precisão' = round(c(loglog_precision_cv[1,1], loglog_precision_smote[1,1],rf_precision_cv[1,1],rf_precision_smote[1,1],xgb_precision_cv[1,1],xgb_precision_smote[1,1], prec_bayes),3),
  'Recall' = round(c(loglog_recall_cv[1,1],loglog_recall_smote[1,1], rf_recall_cv[1,1],rf_recall_smote[1,1], xgb_recall_cv[1,1],xgb_recall_smote[1,1], recall_bayes),3),
  
 'F1Score' = round(c(loglog_f1_cv[1,1],loglog_f1_smote[1,1], rf_f1_cv[1,1],rf_f1_smote[1,1], xgb_f1_cv[1,1],xgb_f1_smote[1,1],f1_bayes),3)
)
