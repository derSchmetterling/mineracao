...
for (cv in seq(1:cv)) {
  ...
  dataTest <- full_data[dataTestIndex,]

  # everything else to train
  dataTrain <- full_data[-dataTestIndex,]
...
m1_blogit <- jags(data=jagsData,model.file='models/m1_cauchit.bug',
                   ...
                   n.chains=2, n.iter=10000, n.burnin=3000, n.thin=10)
```
...
pred_testb <- predict_bcauchit(samples,data_test)


roc_bayes <- Epi::ROC(pred_testb, data_test$y, plot= F)
...
```{r}
summary(samples)$statistics[, 'Mean']
```
...
over_train <- SmoteClassif(y ~ MEN + URBAN+ AGE + SENIORITY, data_reg, k = 5, C.perc = list('1' = 1.75,'0' =1), repl = FALSE,
dist = "HEOM", p = 2)
